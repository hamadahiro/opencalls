...
function splitPrizeParts(prize) {
  if (!prize) return [];
  return prize.split(' + ').map(function(s) { s = s.trim(); return s.charAt(0).toUpperCase() + s.slice(1); }).filter(Boolean);
}

function parsePrizeAmount(text) {
  // Check for pipe first — if present, always split on it
  var pipe = text.indexOf('|');
  if (pipe > 0) {
    var head = text.slice(0, pipe).trim();
    var lbl = text.slice(pipe + 1).trim();
    var m = head.match(/^(\d+\s*×\s*)?(US\s*)?[€£$¥][\d,]+\+?$/);
    if (m) { return { amount: head, label: lbl || null }; }
    return { amount: null, headline: head, label: lbl };
  }
  // No pipe — try monetary regex
...
function renderInfoGrid(call) {
  ...
  if (call.prize) {
    ...
    const cardsHtml = parts.map(part => {
      ...
      if (p.headline) {
        return `<a href="${href}" class="prize-card"><span class="prize-amount">${esc(p.headline)}</span><span class="prize-label">${esc(p.label)}</span></a>`;
      }
